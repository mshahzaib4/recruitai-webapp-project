...
from scorer import rank_resumes, rank_with_uploaded, rank_with_bulk_uploaded
...
async def match_resumes(body: MatchRequest, request: Request):
    ...
    jd_embedding = await embedder.embed(body.job_description)
    raw = rank_resumes(jd_embedding, embedder.embeddings, embedder.data, top_k=body.top_k)
    jd_skills = extract_skills(body.job_description)
    results = [CandidateResult(**raw) for raw in raw]
...
async def upload_and_match(
    ...
    jd_embedding = await embedder.embed(job_description)
    uploaded_embedding = await embedder.embed(resume_text)
